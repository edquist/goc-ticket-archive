...
#determine if it's a local user
...
open (LOCALUSERS, "/etc/grid-security/dn.localuser") || warn "Can't open local user list\n";
...
$localuser = 1 if grep(/$identity/, @localusers);
  

#my $localuser = `grep "$identity" /etc/grid-security/dn.localuser`;
#end of determine...
...
sub submit
{
    ...
    my $is_glidein = 0;
    my $is_monitoring = 0;
    my $mydistro = '( (Distro == "RHEL5") || (Distro == "RHEL6") )';
    ...
    if (scalar(@tmpr) > 0)
    {
        $mydistro  = "(" .
            ...
            ")";
        #push(@requirements, $r);
      #$rc = open(MYFILE, ">/tmp/distrotest") ; 
      #print MYFILE $r;
      #close(MYFILE); 
    }
    #elsif (defined($condor_distro))
    #{
    #    my $r = "(" . join(" || ",
    #        map { "OpSys == \"$_\"" } split(/\s+/, $condor_distro)) . ")";
    #    push(@requirements, $r);
    #}
    ...
    print SCRIPT_FILE "periodic_remove = ((CurrentTime -EnteredCurrentStatus) > 3600*48)\n";
    #mark the local user jobs,May 28/2008
    if ( $localuser ) {
        print SCRIPT_FILE "+localCMSJob = TRUE\n";
    }
    if ($logname =~ /cmsprio\d*/) {
       print SCRIPT_FILE "+PriorityCMSJob = TRUE\n"
    }
    if ($logname =~ /cmsprod\d*/) {
       print SCRIPT_FILE "+AccountingGroup = \"group_cmsprod.$logname\"\n";
    }
    if ( $logname =~ /uscms\d\d/ || $logname =~ /cms\d\d\d\d/ || $logname =~ /cmsprio\d*/ ) {
       print SCRIPT_FILE "+AccountingGroup = \"group_cmsanalysis.$logname\"\n";
    }
    
    # is the job a grid monitoring job -- Thu Sep 20 10:36:14 EDT 2012 -- fhu
    $is_monitoring = 1 if ($logname eq "cms0002");
    # is the job a glidein job -- Thu Sep 20 10:36:14 EDT 2012 -- fhu
    $is_glidein = 1 if ($identity =~ /pilot/);
    if($logname eq "cms0020" )
    {
         open(MYFILE, ">>/tmp/identity") ; 
        print MYFILE "id:".$identity."\n";
        print MYFILE "cmd:".$proxy_info_cmd."\n";
        print MYFILE "proxy:"."@proxy_info"."\n";
       close(MYFILE); 
    }
        

    if ($logname =~ /.*cms.*/) { 
        print SCRIPT_FILE "+CMSJob = TRUE\n";
        #print SCRIPT_FILE "+AccountingGroup = \"group_cms\"\n";
        #print SCRIPT_FILE "Rank = ( ClusterName == \"CMS\" ) * 100 \n";  
        print SCRIPT_FILE "Rank = ( ( ClusterName == \"CMS\" ) * 300 ) + ( ( ClusterName == \"Rossmann\" ) * 200 ) + ( ( ClusterName == \"Steele\") * 100 ) \n";
        if( $is_glidein | $is_monitoring ){
          print SCRIPT_FILE " Requirements = ( ( Target.FileSystemDomain == \"steele-cgf.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"bluearc.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"lustrea.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"lustrec.rcac.purdue.edu\" ) ) && ( Arch == \"x86_64\" ) && ( ClusterName == \"CMS\" )  && ( Memory >= 1024 ) && $mydistro \n ";
        } else{
          print SCRIPT_FILE " Requirements = ( ( Target.FileSystemDomain == \"steele-cgf.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"bluearc.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"lustrea.rcac.purdue.edu\" ) || ( Target.FileSystemDomain == \"lustrec.rcac.purdue.edu\" ) ) && ( Arch == \"x86_64\" ) && ( ClusterName != \"Miner\" ) && ( ClusterName != \"Benzene\" ) && ( ClusterName != \"Dryrun\" ) && ( Memory >= 1024 ) && $mydistro \n ";
        }
    } else {
        print SCRIPT_FILE " Requirements =  ( ClusterName != \"Dryrun\" ) && $mydistro \n ";
        print SCRIPT_FILE "+AccountingGroup = \"group_osg\"\n";
    }
    ...
    #system("cp $script_filename /tmp/condorsub");
